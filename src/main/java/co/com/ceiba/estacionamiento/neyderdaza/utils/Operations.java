...
public class Operations {
    ...
    public boolean motorcycleExceedsEngine(int engine){
        return engine>CC_LIMIT;
    }

    public boolean isMotorcycle(String vehicleType){
        return vehicleType.equalsIgnoreCase(MOTORCYCLE_TYPE);
    }

    public boolean isCar(String vehicleType){
        return vehicleType.equalsIgnoreCase(CAR_TYPE);
    }

    public  double calculateMotorcycleParkingWithExtraMoney(int daysToPay, int hoursToPay){
        return (double)(daysToPay * MOTORCYCLE_VALUE_DAY) + (hoursToPay * MOTORCYCLE_VALUE_HOUR) + EXTRA_MONEY_TO_PAY_FOR_CC_LIMIT;
    }

    public double calculateMotorcycleParking(int daysToPay, int hoursToPay){
        return (double)(daysToPay * MOTORCYCLE_VALUE_DAY) + (hoursToPay * MOTORCYCLE_VALUE_HOUR);
    }

    public double calculateCarParking(int daysToPay, int hoursToPay){
        return (double)(daysToPay * CAR_VALUE_DAY) + (hoursToPay * CAR_VALUE_HOUR);
    }

    public int divisionPerTotalHourPerDay(int parkingHours){
        return parkingHours / TOTAL_HOURS_PER_DAY;
    }

    public int modulePerTotalHourPerDay (int parkingHours){
        return parkingHours % TOTAL_HOURS_PER_DAY;
    }
}
